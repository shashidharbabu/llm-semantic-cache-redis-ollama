...
class DatabaseWithCache:
    ...
    def _setup_database(self):
        ...
        # Ensure a 'views' column exists for syncing view counters
    ...
    def increment_post_views(self, post_id: int) -> int:
        ...
        - Uses Redis INCR on key "views:post:{post_id}"
        - Every 10 views, syncs the count back to SQLite `users.views`
        """
        cache_key = f"views:post:{post_id}"
        # Increment in Redis
        view_count = self.redis_client.incr(cache_key)

        # Every 10 views, persist to DB
        if view_count % 10 == 0:
            self.cursor.execute("UPDATE users SET views = ? WHERE id = ?", (int(view_count), post_id))
            self.conn.commit()
